...
includeFile = function (file, priority) {
	...
	try {
		...
		Object.keys(localActions).forEach(function (name) {
			...
			if ( typeof (action.js) === 'string' ) {
				...
				watchers.push(fs.watch(action.executable, update));
...
var update = _.throttle(updateSync, 1000);
...
var actionsDirectoriesQueue = async.queue(function (task, callback) {
	...
	fs.readFile(counterFile, function (err, data) {
		...
		fs.mkdirs(libpath.join(filesRoot, outputDirectory), function (err) {
			if ( err ) {
...
function RPC(POST, callback) {
	...
	var header = "[" + this.id + "] ";
	this.log = function (msg) {log(header + msg)};
...
RPC.prototype.parseParameter = function (parameter, cb) {
	...
	case 'file':
	case 'directory':
		var path = libpath.join(filesRoot, value);
...
RPC.prototype.handleOutputDirectory = function (callback) {
	...
	case "" : 
		...
		fs.mkdirs(libpath.join(filesRoot, this.outputDirectory), callback);
		break;
	default :
...
function getAvailableMemory() {
	availableMemoryRatio = os.freemem() / totalMem;
	setTimeout(getAvailableMemory, 2000);
}
getAvailableMemory();
...
RPC.prototype.executeAction = function (callback) {
	...
	if ( typeof (js) === "object" ) {
		...
		actionParameters2.output_directory =  libPath.join(filesRoot, this.outputDirectory);
