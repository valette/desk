qx.Class.define("desk.action", 
{
	...
	construct : function (actionName, providedParameters, fileBrowser)
	{
		this.__actions=desk.actions.ACTIONSHANDLER;
		var action=this.__actions.getActionsXMLElement().getElementsByName(actionName)[0];
...
//			var executionStatus=new qx.ui.basic.Label();
		...
		manager.addListener("complete", function() {
			...
			if (manager.getValid()) {
				...
				this.__actions.launchAction (parameterMap, getAnswer, this)
			} else {
